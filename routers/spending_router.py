...
from models.spending_models import Spending, SpendingCreate
...
# ✅ 날짜별 소비 총액 (같은 기능, URL 구분용 백업)
@router.get("/spending/all")
async def get_all_spendings(token: str = Depends(oauth2_scheme)):
    payload = verify_token(token)
    user_id = payload.get("sub")
